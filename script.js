document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initializeTrackerPage(userData) {
        ...
        async function runSignalAnalysis() {
            ...
            const coinsInput = document.getElementById('signalAnalysisCoins').value;
            settings.signalAnalysisCoins = coinsInput;
            localStorage.setItem('signalAnalysisCoins', coinsInput);

            const params = {
                coins: coinsInput.split(',').map(s => s.trim().toUpperCase()).filter(Boolean),
                ...
                rsiThreshold: parseFloat(document.getElementById('signalAnalysisRsi').value) || null,
                adxThreshold: parseFloat(document.getElementById('signalAnalysisAdx').value) || null,
            };
            ...
            try {
                ...
                for(const coin in data) {
                    ...
                    if(res.error) {
                        ...
                        // Sonuçları ve "Alarma Ekle" butonunu göster
                    }
                    html += `</div>`;
                }
        ...
        function addEventListeners() {
            ...
            document.getElementById('refreshBtn').addEventListener('click', async () => { await fetchAllDataAndRender(); await fetchAiDataAndRender(); });
            ...
            document.getElementById('strategyPresetFilters').addEventListener('click', e => {
                if (e.target.classList.contains('strategy-btn')) handleStrategyPresetClick(e.target.dataset.preset);
            });
            document.getElementById('portfolio-tabs-container').addEventListener('click', e => {
                if (e.target.classList.contains('tab-link')) setActivePortfolio(e.target.dataset.portfolioName);
            });
            document.getElementById('runSignalAnalysisBtn').addEventListener('click', runSignalAnalysis);
        }
    });
